...
int can_hold_my_bag(bag_ht* bht, char* name) {
	bag* b;
	...
	int res = 0;
	for (int i = 0; i < b->count; i++) {
		if (strcmp(b->holds[i], "shinygold") == 0) {
			return 1;
		}
		res |= can_hold_my_bag(bht, b->holds[i]);
	}
	return res;
}
...
int main(int argc, char *argv[])
{
	...
	int sum = 0;
	for (int i = 0; i < bht->key_count; i++) {
		if ((b = get_bag_ht(bht, bht->keys[i])) != NULL) {
			sum += can_hold_my_bag(bht, bht->keys[i]);
		}
	}
