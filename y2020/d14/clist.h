...
{ \
	...
	dlist->data = (type*)calloc(1,sizeof(int)*cap); \
...
inline void append_##type##_list(type##_list* dlist, type x) \
{ \
...
inline void set_##type##_list(type##_list* dlist, int i, type x) \
{ \
	if (i < 0) { return; } \
	if (i >= dlist->cap) { \
		int oc = dlist->cap; \
...
inline type get_##type##_list(type##_list* dlist, int i) \
{ \
	if (i >= 0 && i < dlist->size) { \
		return dlist->data[i]; \
	} \
...
inline void print_##type##_list(type##_list* dlist) \
{ \
...
inline void print_long(long x)
{
	printf("%ld", x);
}
DefineListFor(long, print_long, 0);

// ulong_list
typedef unsigned long u_long;
void print_ulong(ulong x);
inline void print_ulong(ulong x)
{
	printf("%lu", x);
}
DefineListFor(ulong, print_ulong, 0);

// float_list
void print_float(float x);
inline void print_float(float x)
{
	printf("%.4f", x);
}
