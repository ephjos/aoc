...
#define GET_CHT_D( d, key, def )    get_cht_d(d, key, def)
#define GET_CHT_ID( d, key, def )  get_cht_id(d, key, def)
...
double get_cht_core(cht* d, char* key, double def);
double get_cht(cht* d, char* key);
double get_cht_i(cht* d, ulong h);
double get_cht_d(cht* d, char* key, double def);
double get_cht_id(cht* d, ulong h, double def);
...
inline void insert_cht_core(cht* d, char* key, double value)
{
	...
	if (d->items[h]) { // collision, append to linked list
		cht_item* curr = d->items[h], *prev;
		while (curr != NULL) {
			if (strcmp(curr->key, key) == 0) { // overwrite
				curr->value = value;
				free(n);
				return;
			}
...
inline double get_cht_core(cht* d, char* key, double def) {
	ulong h = hash_cht(key);

	// Check if bucket exists
	if (!d->items[h]) return def;
...
{
	return get_cht_core(d, key, NOT_FOUND);
}

inline double get_cht_d(cht* d, char* key, double def)
{
	return get_cht_core(d, key, def);
}
...
inline double get_cht_i(cht* d, ulong h)
{
	char* buf = (char*)malloc(sizeof(char)*BUF_SIZE);
	sprintf(buf, "%lu", h);
	double res =  get_cht_core(d, buf, NOT_FOUND);
	free(buf);
	return res;
}

inline double get_cht_id(cht* d, ulong h, double def)
{
	char* buf = (char*)malloc(sizeof(char)*BUF_SIZE);
	sprintf(buf, "%lu", h);
	double res =  get_cht_core(d, buf, def);
	free(buf);
	return res;
}
