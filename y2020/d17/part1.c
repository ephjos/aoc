...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			...
			space[c][i+(o/2)][j+(o/2)] = lines[i][j] == ALIVEC;
		}
	}
			...
			for (int j = 0; j < N; j++) {
				for (int k = 0; k < N; k++) {
					...
					for (int x = -1; x <= 1; x++) {
						for (int y = -1; y <= 1; y++) {
							for (int z = -1; z <= 1; z++) {
								if (x == y && y == z && z == 0) continue; // skip self
								if (IS_BETWEEN(i+x, 0, N-1) && IS_BETWEEN(j+y, 0, N-1) &&
										IS_BETWEEN(k+z, 0, N-1)) {
									alive_neighbors += p_space[i+x][j+y][k+z];
								}
							}
						}
					}
					n_space[i][j][k] = ((p_space[i][j][k] && (alive_neighbors == 2 || alive_neighbors == 3))
								|| (!p_space[i][j][k] && alive_neighbors == 3));
				}
			}
		}
